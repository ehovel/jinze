...
class NSP_GK4_Layout_Parts {
	...
	function lists($config, $news_id, $news_cid, $news_title, $news_text, $odd, $num,$news_date) {
		if($config['news_short_pages'] > 0) {
			...
			$title = str_replace('"', "&quot;", $title);
			$link = ($news_id !== 0) ? JRoute::_(ContentHelperRoute::getArticleRoute($news_id, $news_cid)) : JRoute::_('index.php?option=com_user&view=login');
			if(JString::strlen($title) > 0) $title = '<a href="'.$link.'" title="'.htmlspecialchars($news_title).'">'.$title.'</a>';
			// creating rest news list
			return $title.'<span class="date">'.date('Y-m-d',strtotime($news_date)).'</span>';	
		}
	}
    /** K2 elements **/
	...
	function image_vm($config, $news_id, $news_cid, $news_image, $news_title){		
		...
		if($IMG_SOURCE != '' && $config['news_content_image_pos'] != 'disabled') {
			...
			//
			if($config['news_image_link'] == 1) {
				return ($config['news_content_image_pos'] == 'center') ? '<div class="center'.$class.'"><a href="'.$IMG_LINK.'"><img class="nspImage" src="'.$IMG_SOURCE.'" alt="'.htmlspecialchars($news_title).'" style="'.$size.'"  /></a></div>' : '<a href="'.$IMG_LINK.'"><img class="'.$class.'" src="'.$IMG_SOURCE.'" alt="'.htmlspecialchars($news_title).'" style="'.$size.'"  /></a>';
			} else {
