 ...
 * Dashboard
 */
$menu->addChild(new JMenuNode(JText::_('MOD_MENU_DASHBOARD'), 'index.php', 'dashboard'));
...
{
	$menu->addChild(
	new JMenuNode(JText::_('MOD_MENU_ARTICLES')), true
	);
...
if ($user->authorise('core.manage', 'com_menus'))
{
	...
	{
		//$titleicon = $menuType->home ? ' <span class="default">*</span>' : '';
		$alt = '*' .$menuType->sef. '*';
		if ($menuType->home == 0)
		{
			$titleicon = '';
		...
		{
			$titleicon = ' <span class="default">*</span>';
		}
...
{
	$menu->addChild(new JMenuNode(JText::_('MOD_MENU_EXTEND'),'','extendmenu'), true);
	...
	if ($components) {
		...
		foreach ($components as &$component) {
			if (is_array($this->params->get('blacklist')) && !in_array($component->element, $this->params->get('blacklist'))) {
...
if ($user->authorise('core.admin')) {
	$menu->addChild(new JMenuNode(JText::_('MOD_MENU_CONFIGURE'), 'index.php?option=com_config', 'class:config'));
}
