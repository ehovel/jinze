...
*/			default :
	...
	function submit()
	{
		...
		if ($params->get('contactmap_afficher_captcha')&&$user->guest) { 
			if(count($_POST)>0){
		...
		// Prepare email body
        $myMessage = JText::_('GMAPFP_EMAIL_RECU_DE').' '.$name.'<'.$email.'> ';
        $myMessage.= JText::_('GMAPFP_EMAIL_RECU_PAR').' '.$SiteName.' '."\r\n\r\n".stripslashes($body);
		...
		if ( $emailCopy && !JError::isError($sent) )
		{
