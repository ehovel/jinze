...
class ContactMapsModelContactMap extends JModel
{
    function __construct() 
    {
        ...
        {
            /** verifi que la fonction n'est d�fini qu'une faois */
        ...
        if (!defined( '_JOS_GMAPFP_APIV3' ))
        {
            ...
            $doc->addCustomTag( '<script type="text/javascript" src="'.JURI::base().'components/com_contactmap/libraries/map.js"></script>'); 
        }
    ...
    function _getQuery()
    {
        ...
        $select = 'a.*, cc.access as category_access, cc.title as category_name, cc.description as cat_description,'
    ...
    //traitement des donn�es de zoom
...
//Cr�ation des marqueurs
    ...
    if ($_layout == "map") {
        ...
        $plus_detail = JTEXT::_("GMAPFP_CLICK_SEND_MAIL");
    }else{
...
//affichage de plus de d�tail : Panoramino, Wiki, Youtube, ...
	$control ="";
	$event_panoramino = "";
...
//selection du type d'affichage des types de carte
    ...
    foreach($rows as $row){
        if (($row->glat<>"")&&($row->glng<>"")) {
            ...
            $image ='';
            if (@$row->image) {
                $image = JURI::base().'/images/contactmap/'.$row->image;
            ...
            $carte.='                   
                ...
                <form action="#" onsubmit="CalculRoute(\''.$num.'\'); return false;" method="post" name="direction_form">
                        ...
                        <p>
                            ...
                            '.JText::_( 'GMAPFP_OU' ).'<input type="text" name="text_from'.$num.'" id="text_from'.$num.'" />
                        </p>
                            '.JText::_( 'GMAPFP_VERS' ).' :
                            ...
                            '.JText::_( 'GMAPFP_OU' ).'<input type="text" name="text_to'.$num.'" id="text_to'.$num.'" />
                            <input type="submit" value='.JText::_( 'GMAPFP_GO' ).' />
                        </div>
    ...
    foreach($rows as $row) {
        ...
        if (($row->image)&&($config->get('contactmap_photo_icon'))) {
			$image = JURI::base().'images/contactmap/thumb_'.$row->image;
	...
	function verif_icon($thumbs)
	{
		...
		foreach ($thumbs as $thumb)
		{
            $image ='';
            if (@$thumb->image) {
				$filepath = JPath::clean(JPATH_ROOT.DS.'images'.DS.'contactmap'.DS.$thumb->image);
				$thumbpath = JPath::clean(JPATH_ROOT.DS.'images'.DS.'contactmap'.DS.'thumb_'.$thumb->image);
